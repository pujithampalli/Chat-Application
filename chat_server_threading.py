...
class ChatServer(threading.Thread):
    ...
    def run_thread(self, username, encrypted_passphrase, conn, addr):
    ...
    def run(self):
        ...
        while True:
            ...
            if (username not in self.connections):
                ...
                print(keyid, "is the keyid sent with username)
                encrypted_passphrase = gpg.encrypt("happy", keyid)
                # Need to send the encrypted session passphrase based on the keyid sent with username
                threading.Thread(target=self.run_thread, args=(username, encrypted_passphrase, conn, addr)).start()
            else:
